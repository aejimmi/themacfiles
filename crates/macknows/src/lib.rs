...
use crate::schema::{
    AppInsight, DecodedRecord, EventInfo, Insights, MlModelInsight, Summary,
};
...
pub fn summary(config_path: &Path, state_path: &Path) -> Result<Summary> {
    ...
    let insights = extract_insights(&records);
...
fn extract_insights(records: &[DecodedRecord]) -> Insights {
    ...
    for r in records {
        let ev = r.event_names.first().map(String::as_str).unwrap_or("");
        let fields = &r.fields;

        // App usage
        if ev == "com.apple.osanalytics.appUsage" {
            if let Some(desc) = field_str(fields, "appDescription") {
                if field_str(fields, "foreground").as_deref() == Some("YES")
    ...
    insights.apps.sort_by(|a, b| b.active_seconds.cmp(&a.active_seconds));
    insights
}
